...
function comp($a, $b)
{
    //essentially, check if the sqrt of each index in b equals a number in a
    $arr = [];
    foreach($b as $num)
    {
       array_push($arr, array_search(sqrt($num), $a, false));
    }

echo count($b);
echo sizeof(array_filter($arr));
print_r($arr);

    if(count(array_filter($arr)) == count($b))
    {
        return 'true';
    }
    return 'false';
